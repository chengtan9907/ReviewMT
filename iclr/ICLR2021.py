...
def get_paper_and_review(pdf_outpath, review_outpath):
    ...
    submissions = client.get_all_notes(invitation='ICLR.cc/2021/Conference/-/Blind_Submission', details='directReplies')
    paper_number = 1
    for submission in tqdm(submissions, desc="ICLR2021_get_paper_and_review", position=1, leave=False):
        # Get paper pdf
        continue_flag = False
        while True:
            try:
        ...
        if continue_flag:
            continue
        with open(pdf_outpath+os.sep+f"ICLR_2021_paper_{paper_number:04d}.pdf", 'wb') as fp:
        ...
        with open(review_outpath+os.sep+f"ICLR_2021_review_{paper_number:04d}.json", 'w', encoding='utf-8') as fp:
