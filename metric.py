...
parser.add_argument("path2yaml", default="./configs/metric.yaml", type=str, help="Path to the arguments yaml")

def calculate_bert_score(reference, hypothesis):
    score = bert_score([reference], [hypothesis], lang='en', device='cuda' if torch.cuda.is_available() else 'cpu')
    return score
...
def process_files(path, dataset, total=100):    
    ...
    for file in os.listdir(path):
        ...
        for idx, (role, gt_reply, pr_reply) in enumerate(zip(roles, gts, prs)):
            ...
            # bert score
            bert_score = calculate_bert_score(gt_reply, pr_reply)
    ...
    return {
        ...
        'bert_score_mean': np.mean(bert_score_list),
        'bert_score_std': np.std(bert_score_list),
...
def main():
    ...
    if 'total_papers' in y:
        ...
        raise Exception("format wrong in config file")
    const_datasets = ['zeroshot', 'finetune']
    const_model_names = [
        'llama3', 
        'qwen', 
        'baichuan2', 
    ...
    for dataset in datasets:
        ...
        for model_name in model_names:
            ...
            print(f"BERT Score Mean: {results['bert_score_mean']:.2f}, Std: {results['bert_score_std']:.2f}")
