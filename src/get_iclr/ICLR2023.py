...
def get_paper_and_review(pdf_outpath, review_outpath):
    ...
    paper_number = 1
    for submission in tqdm(submissions, desc="ICLR2023_get_paper_and_review", position=1, leave=False):
        # Get paper pdf
        continue_flag = False
        while True:
            try:
        ...
        if continue_flag:
            continue
        with open(pdf_outpath+os.sep+f"ICLR_2023_paper_{paper_number:04d}.pdf", 'wb') as fp:
        ...
        for r in Official_Review:
            ...
            meta_review = {
                ...
                'strengths_and_weakness': r['content']['strength_and_weaknesses'],
                    ...
                    if r['signatures'][0].endswith("Authors"):# write by author
                        ...
                        for j in meta_paper_info['reviewers']:
                            if r['signatures'][0].endswith(j['name']):
                                ...
                                break
        for r in Decision:
                meta_paper_info['meta_review'] = r['content']['metareview:_summary,_strengths_and_weaknesses']
                meta_paper_info['decision'] = r['content']['decision']
        with open(review_outpath+os.sep+f"ICLR_2023_review_{paper_number:04d}.json", 'w', encoding='utf-8') as fp:
