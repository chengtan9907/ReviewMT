...
def get_paper_and_review(pdf_outpath, review_outpath):
    ...
    submissions = client.get_all_notes(invitation='ICLR.cc/2019/Conference/-/Blind_Submission', details='directReplies')
    paper_number = 1
    for submission in tqdm(submissions, desc="ICLR2019_get_paper_and_review", position=1, leave=False):
        # Get paper pdf
        continue_flag = False
        while True:
            try:
        ...
        if continue_flag:
            continue
        with open(pdf_outpath+os.sep+f"ICLR_2019_paper_{paper_number:04d}.pdf", 'wb') as fp:
        ...
        Decision = [reply for reply in submission.details["directReplies"] if reply["invitation"].endswith("Meta_Review")]
                    ...
                    if r['signatures'][0].endswith("Authors"):# write by author
                        ...
                        for j in meta_paper_info['reviewers']:
                            if r['signatures'][0].endswith(j['name']):
                                ...
                                break
        for r in Decision:
                meta_paper_info['meta_review'] = r['content']['title'] + '\n' +r['content']['metareview']
                meta_paper_info['decision'] = r['content']['recommendation']
        with open(review_outpath+os.sep+f"ICLR_2019_review_{paper_number:04d}.json", 'w', encoding='utf-8') as fp:
